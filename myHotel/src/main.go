// future me, try using recursion instead of returning zeros so in case of invalid input the function calls itself again
package main
...
func checkErr(err error, clue string) {
	if err != nil {
		clearScr()
		fmt.Println(clue + " usage caused an error")
		showCursor()
		panic(err)
	}
}
...
func logOrder(bill map[int]Bill, username string) {
	var fileName string
	timeFormat := time.Now().Format("January_2__Monday__3:4:5_PM__Username[")
	if len(username) != 0 {
