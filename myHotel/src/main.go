...
func takeOrder(menu map[int]Menu) map[int]Bill {
	...
	var stopper bool
	fmt.Println(pterm.DefaultBox.WithTitle("Instructions: ").Sprintfln("* Enter %s to add the dish to the cart\n* Enter %s to checkout\n* Enter %s to quit", pterm.Bold.Sprint(pterm.FgMagenta.Sprint("DISH NUMBER")), pterm.Bold.Sprint(pterm.FgGreen.Sprint("DONE")), pterm.Bold.Sprint(pterm.FgRed.Sprint("EXIT"))))
	for !stopper {
...
func printBill(bill map[int]Bill) int {
	...
	table.Render()
	fmt.Print("\n")
	fmt.Println(pterm.DefaultBox.Sprintln("Total Cost:", pterm.NewStyle(pterm.FgGreen, pterm.Bold).Sprint("₹ ", strconv.Itoa(total))))
	return total
}
...
func letsDel(bill map[int]Bill) map[int]Bill {
	...
	var history []int
	billLen := len(bill)
	pterm.DefaultBox.WithTitle("Instructions: ").Printfln("* Enter %s to remove the dish from the cart\n* Enter %s to save changes\n* Enter %s to quit", pterm.Bold.Sprint(pterm.FgMagenta.Sprint("DISH NUMBER")), pterm.Bold.Sprint(pterm.FgGreen.Sprint("DONE")), pterm.Bold.Sprint(pterm.FgRed.Sprint("EXIT")))
	fmt.Print("\n")
	for !stopper {
		...
		default:
			...
			if !skipFlag {
				if bill[choice].Qty == 1 {
					pterm.FgLightGreen.Printf("removed %s from cart\n\n", bill[choice].Name)
					...
					var stopper02 bool
					for !stopper02 {
						fmt.Printf("There are %d %s in cart, enter how many to remove: ", bill[choice].Qty, bill[choice].Name)
						...
						if howMany == 0 {
							...
							if entry, ok := bill[choice]; ok {
								...
								entry.Qty -= howMany
								bill[choice] = entry
								pterm.FgLightGreen.Printfln("%d %s now left in cart", bill[choice].Qty, bill[choice].Name)
							}
						}
					}
				}
...
func askToPay(price int) bool {
	var stopper bool
	pterm.DefaultCenter.Print(pterm.DefaultBox.Sprint(pterm.NewStyle(pterm.Bold, pterm.FgCyan).Sprintf("Proceed to pay: %s, Confirm with yes or no, Enter 'exit' to quit", pterm.NewStyle(pterm.FgGreen, pterm.Bold).Sprint("₹ ", strconv.Itoa(price)))))
	for !stopper {
		choice := userInputHandler01()
		if choice == 0 {
			continue
		...
		case 1:
			clearScr()
			spinnerLiveText, err := pterm.DefaultSpinner.Start("Processing Transaction")
			checkErr(err, "spinner usage caused an error")
			time.Sleep(time.Second * 4)
			spinnerLiveText.UpdateText("Almost Done")
			time.Sleep(time.Second * 2)
			spinnerLiveText.Success("Transaction Successful")
			return true
		case 2:
			quit()
		}
...
func showETA(bill map[int]Bill) {
	...
	clearScr()
	if len(username) != 0 {
		pterm.Info.Printf("Currently preparing for: %s", username)
		fmt.Print("\n")
	}
	var eta int
	for i := range bill {
