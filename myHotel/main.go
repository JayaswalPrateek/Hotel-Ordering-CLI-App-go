		...
		for decider {
			if choice != 0 {
				...
				default:
					...
					for i := range bill {  // check if input dish is already in bill, if yes then update its existing qty
						if bill[i].Name == menu[choice].Name { // if ordered dish already in bill, update qty of the dish in the bill
							if entry, ok := bill[i]; ok {
								entry.Cost += (entry.Cost / entry.Qty) // update cost
